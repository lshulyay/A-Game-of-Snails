...
ig.global.EntityRacelistGUI = ig.Entity.extend({
    ...
    initialize: function() {
        ...
        for (var i = 0; i < this.listLimit; i++) {
            var race = ig.game.controller.raceListArr[i];
            if (race) {
                ig.game.pool.useObject('trigger', {kind: 'singleRace', parentEntity: this, temp: {raceID: race._id, raceName: race.name, raceFinished: race.finished, countDown: false, entrantsArr: race.entrantsArr, reqEntrants: race.reqEntrants}, pos: {x: x, y: y}});
                y += 30;
            }
        }
    ...
    refreshRaceEntrants: function(snail) {
        ...
        if (!this.openRace.finished && this.openRaceEntrantsArr.length >= this.openRace.reqEntrants) {
    ...
    scrollUp: function() {
        ...
        if (race) {
            ...
            if (race) {
                ...
                var x = ig.game.gui.menuPanel2Pos.x;
                var y = ig.game.gui.menuPanel2Pos.y + 55;
                ig.game.pool.useObject('trigger', {kind: 'singleRace', parentEntity: this, temp: {raceID: race._id, raceName: race.name, raceFinished: race.finished, countDown: true, entrantsArr: race.entrantsArr, reqEntrants: race.reqEntrants}, pos: {x: x, y: y}});
            }
        }
    },
