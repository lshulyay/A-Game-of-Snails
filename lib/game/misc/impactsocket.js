	...
	ig.ImpactSocket = ig.Class.extend({
		...
		loginUser: function() {
			...
			if (data.username && data.password) {
            	...
            	// If session username was successfully set...
            	/* if (ig.game.sessionUsername !== null) {
            		// Close access prompt
					ig.game.controller.toggleAccessPropmt();
            	//	ig.game.loadLevel(LevelMain);
            	} */
			}
		},
		...
		saveSnail: function(snail) {
			...
			/* var resolve = function(data) {
				snail.snailID = data;
				console.log('snail ID set: ' + snail.snailID);
			...
			// this.socket.on('setSnailID', resolve);
			/* this.socket.on('setSnailID', function(snailid) {
				snail.snailID = snailid;
				console.log('snail ID set: ' + snailid);
		...
		loadRacelist: function(cb) {
			...
			this.socket.emit("loadracelist", finished, function(err, races) {
				...
				/*	for (var i = 0; i < races.length; i++) {
					...
					// Set username based on form input and ID based on DB info
	    				ig.game.controller.raceListArr.concat(race);
	    				// ig.game.gui.populateRaceList();
	    			} */
		...
		runRace: function(race,entrants) {
			...
			this.socket.emit("runrace", data, function(err,raceResultsArr) {
				if (err) {
					...
					this.updateRace(race._id,'resultsArr',raceResultsArr);
				}
			});
		}
